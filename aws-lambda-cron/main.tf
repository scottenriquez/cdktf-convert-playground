...
resource "aws_lambda_function" "local_zipfile" {
  ...
  source_code_hash = var.function_s3_bucket == "" ? "${filebase64sha256("${var.function_zipfile}")}" : ""
...
locals {
  function_id         = "${element(concat(aws_lambda_function.local_zipfile.*.id, [""]), 0)}${element(concat(aws_lambda_function.s3_zipfile.*.id, [""]), 0)}"
  function_arn        = "${element(concat(aws_lambda_function.local_zipfile.*.arn, [""]), 0)}${element(concat(aws_lambda_function.s3_zipfile.*.arn, [""]), 0)}"
  function_invoke_arn = "${element(concat(aws_lambda_function.local_zipfile.*.invoke_arn, [""]), 0)}${element(concat(aws_lambda_function.s3_zipfile.*.invoke_arn, [""]), 0)}"
}
